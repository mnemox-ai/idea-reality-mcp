...
2. **quick vs deep 取捨**：quick 只 2 源 → 競品偏薄；deep 6 源但**冷啟動 28s 卡在漏斗入口**。方案：
   - AngelRun 入口改吃 **deep 的快取版**（背景預熱 + 結果快取），或
   - quick 升到 3-4 源（加 npm/PyPI，去掉最慢的 PH/SO），平衡速度與涵蓋。**決策點見 §7-B。**
...
- **A. 引擎契約怎麼給**（工程偏向、但影響 client）：`/api/check` 加 `include=` 旗標（不破既有）vs 新開 versioned `/api/check2`。**建議：加 `include=` 旗標**（最小破壞，AngelRun 帶旗標拿完整）。→ 這題我傾向直接做，除非你反對。
- **B. quick 的涵蓋 vs 速度**（產品體感）：AngelRun 漏斗入口要「快（2-3 源、<3s）」還是「全（6 源、要快取暖機）」？**建議：入口先給 quick 3-4 源 + 背景補 deep**，體感快、資料不薄。
- **C. Demand Radar 的護城河/變現**（Kevin 級商業）：公開到什麼程度？免費/付費怎麼切？**建議：主題級趨勢公開當磁鐵、想法級深度 gated 導 AngelRun/變現。** 這題牽動 idea-reality 是否重新收費 → 要跟 Kevin 對齊。
...
| **P1 兩邊同吃引擎** | 引擎契約 `include=` + AngelRun 接 demand_heat + 漏斗顯示 | 兩邊 | ~1-2 天 | 決策 A |
