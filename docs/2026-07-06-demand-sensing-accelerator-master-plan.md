...
### 2.0 🔴 P2 實作發現（2026-07-06）：引擎整體慢，不只 quick 涵蓋
做漸進式 scan 時量到真相，比「quick 只有 2 源太薄」嚴重：
- **`/api/check` quick 本身就 14–31s（變動大）**，health 才 1.3s → 慢在應用層不是網路。變動大指向 **source 端（GitHub retry/backoff，token 可能被 rate-limit）**。
- **crowd/demand（語意查詢）從 Render 量 26–31s**。根因＝`vector_distance_cos` 是 **O(N) 全掃 10,150 筆、無索引**（我筆電 HTTP 量約 9s，Render libsql client + demand_heat 附加查詢疊到 ~26s）。
- **LLM**：keyword 抽取 ~3s + pivot hints ~3-5s（可平行/可延後）。
- ⚠️ **P1 的副作用**：AngelRun 同步 ideaCheck 帶 `include:["demand"]` 等於加 ~20s crowd、逼近它 28s timeout → **已撤掉**（`0518fdc`，demandHeat 契約留著待引擎變快）。

**已做的緩解**：漸進式 scan（`POST /api/scan` + `GET /api/scan/{id}`）機制 done——quick 秒回（相對）+ 背景補 deep，**第一層不吃 crowd**（crowd 只在背景 deep 算）。把單次 25s+ 阻塞變「先 14s 出東西 + 背景補完」。但「秒回」的真體感要靠下面的引擎加速。

**引擎加速待辦（2026-07-06 更新後的真實排序 + 進度）**：
- ✅ **Source 延遲（最大宗，已修）＝ GitHub 平行化**（`07f0f92`）。根因**不是** token（`GITHUB_TOKEN` 健康 5000/5000）——是 `search_github_repos` **每個關鍵字循序打 2 個請求**（主搜+近期）。改成 `asyncio.gather` + `Semaphore(5)`（避 GitHub secondary rate limit）。**GitHub 3kw 5.0s→1.8s、5kw ~2.1s**；**prod quick 14-31s → 11-18s**（關鍵字多/限流時改善更大）。16 tests 綠。
- ✅ **Scan flash 首層（已做）**（`a2444a9`）：`_compute_report(flash=True)` 第一層跳過兩個 LLM（dictionary keyword + template pivot）+ 無 crowd → **first paint 14-31s → 3.6s**（prod 實測）。背景 deep 補完整品質（LLM keyword/6 源/LLM pivot/demand），輪詢驗過 competitors 5→11 補進。`meta.partial` 標記。**Skyscanner 漸進式真正成立。**
- 🟡 **剩餘（非阻斷）**：`/api/check`（非 scan 的單次呼叫）仍走完整 LLM ~11-18s——AngelRun 漏斗要更快就改走 `/api/scan`（前端 lane，之後）。
- 🔴 **Turso ANN 向量索引＝放棄（這台建不了）**。crowd 真實延遲**其實才 ~6s 不是 26s**（26s 是我當時猛打 DB 的競爭假象），所以**沒那麼急**。而且 ANN 索引在此環境建不起來：Turso HTTP `/v2/pipeline` **有 ~60s 硬上限**、10k×1536 bulk build 超時斷線；libsql sync client 在 Windows hang；turso CLI 無 Windows 原生支援。**⚠️ 過程踩雷**：試 `DROP COLUMN embedding_vec`（10k 大表 rewrite）在 Turso server 端 churning、一度拖慢 live（502/timeout），已停手、DB 自行 roll back 恢復、**零資料損失**。**DB 現況殘留（無害，live 用 `embedding` 舊欄不受影響）**：`score_history.embedding_vec F32_BLOB(1536)`（10,150 populated）+ orphan `libsql_vector_meta_shadow`/`score_history_emb_idx_shadow`（清不掉、無真 index）。**要建 ANN 要換 Linux env + Turso platform token 走 CLI**，或改走下面的 offline 聚類。
- 🟢 **建議取代 ANN 的路＝offline 需求聚類**（sidestep Turso 索引 + 直接做 Demand Radar §4）：離線 k-means 把 10k embedding 聚成 ~100 主題中心，查詢比對中心（<10ms）。crowd 6s→快 + 是殺手鐧本體。**完整落地計畫＝`docs/2026-07-06-demand-radar-offline-clustering.md`**（架構/資料模型/pipeline/安全邊界/分階段，build-ready）。
...
| **P2 引擎真相** | 🟡 進行中：漸進式 scan 機制 done；**但撞到引擎整體慢（見下）** | idea-reality | ~2-3 天 | 決策 B |
