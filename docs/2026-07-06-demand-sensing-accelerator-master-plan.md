...
**引擎加速待辦（真正的 P2，按預期收益排序）**：
1. **Turso ANN 向量索引**（crowd 26s→ms，同時修好官網 crowd-intel + 解鎖 AngelRun 重接 demand）。✅ 已加 `embedding_vec F32_BLOB(1536)` 欄 + backfill 10,150 筆（in-DB copy，raw BLOB 直接相容）。🔴 **但 `CREATE INDEX ... libsql_vector_idx` 透過 Turso HTTP `/v2/pipeline` 建不起來**（回 `unable to update global metadata table`／大 build 直接斷線）→ **要改從 Render 端（Linux，真 libsql client）或 Turso CLI 建索引**。欄位已就緒，只差建索引 + 把 `search_similar_by_embedding` 改用 `vector_top_k`。
2. **Source 延遲（GitHub retry/backoff）**：查 `GITHUB_TOKEN` 是否 rate-limited/過期；加 source-health 觀測（每源命中率/延遲/錯誤）；給慢源更緊的 timeout。
3. **LLM 平行化**：keyword + pivot 可與 source 搜尋重疊，別串行。
4. **Flash 首層**（可選）：dictionary keyword（省 Haiku 3s）+ template pivot（省 LLM）讓 scan 第一層更快。
