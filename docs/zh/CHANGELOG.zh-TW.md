...
### 新增
- **時間維度信號（Temporal Signals）** — GitHub 儲存庫新鮮度比率、HN 趨勢偵測、Product Hunt 上架時間分析
- **連續評分函數** — 以對數曲線 `min(100, k × ln(1 + count))` 取代階梯式門檻評分
- **`market_momentum` 子維度分數** — 跨來源時間維度比率的加權平均（0-100）
- **趨勢指標** — 根據市場動能判定 `accelerating`（加速）/ `stable`（穩定）/ `declining`（衰退）

### 變更
- **v0.5 評分公式** — 加入時間維度加成（±10 分）；v0.5 分數與 v0.4 **不可比較**
- 子維度分數上限從 90 提高至 100

### 修復
- 漏斗事件追蹤 `sendBeacon` Content-Type bug（MIME type 錯誤）
