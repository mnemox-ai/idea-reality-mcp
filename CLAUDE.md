...
- [2026-07-08] **feat: 公開 Demand Radar `/api/demand-radar` + 抗污染熱度（`e22e892`+`d18bf9b`，Render live 驗過）＝AngelRun 開工序序 2**。公開「大家想叫 AI 蓋什麼」趨勢榜＝SEO 磁鐵 + idea-reality→AngelRun 漏斗頂。**只回聚合**（label + 90d distinct-requester 人數 + trend + 每主題 AngelRun build link），**絕不回 sample_ideas/原文**（T-8，SQL 驗過無 sample_ideas）。
  ...
  - rebuild 跑法：`export TURSO_* && python scripts/build_demand_topics.py --k 100`（讀 embedding 不需 OpenAI；~101 HTTP 寫入超過 bash 2min→用 run_in_background）。**剩 C 官網趨勢頁（前端 lane，多語 tab）+ D 刷新 cron**。
- [2026-07-08] **feat: AngelRun cross-sell CTA（`next_step`）— idea-reality→AngelRun 導流漏斗接上（`a42e932`，Render autoDeploy 部署中）**。＝需求感測加速器 master plan「導流 CTA」/ AngelRun 盤點開工序**序 1**。每次 reality check ＝ build intent 時刻 → 三入口掛 `next_step` 指 AngelRun `/new`，UTM 按 medium 量測：REST `/api/check`+`/api/scan`（medium=api）、MCP `idea_check` tool（medium=mcp）、README banner en+zh-TW（medium=readme）。單一真相來源 `src/idea_reality_mcp/cta.py`；**additive/ignorable 欄位**＝既有 client（含 AngelRun ideaCheck proxy，只 map 特定欄位、`.parse` 在 map 後物件上）不受影響。測試 323 passed（3 既有失敗與本次無關，stash 驗證過）。⚠️ **uvx 安裝的 MCP 用戶要下次 PyPI release 才吃到 tool 的 next_step**；README（GitHub 即時）+ REST（Render）+ HTTP-MCP（Render `/mcp`）已涵蓋主漏斗。轉換數據＝UTM `utm_source=idea-reality`，兩週後看。
